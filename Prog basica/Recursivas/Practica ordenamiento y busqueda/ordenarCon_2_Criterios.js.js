...
let nombres = new Array(dimension);//declaro los arr
...
//cargo los arreglos
cargarNom_Alt_y_edad(nombres, edades, altura );//aca hice un cambio que quiero consultar. 
...
function cargarNom_Alt_y_edad(nombres, edades, altura) {
  for (let i = 0; i < nombres.length; i++) {
    // nombres[i] = readlineSync.question(" Ingrese nombre ");
    nombres[i] = "sujeto "+i;
    edades[i] = getRandomInt(18, 20);//aqui las llamamos
    altura[i] = getRandomInt(150, 201);
  }
}

function getRandomInt(min, max) {//aqui usamos la funcion para cargar edades y alturas aleatorias
...
function ordenarPorEdad(nombres, edades, altura) {//aqui usamos el ejemplo de la facturacion
  for (i = 0; i < edades.length; i++) {
    ...
    let j = 0;
    while (j < i && edades[j] > edad) {//el primero va directo hacia abajo
      j++;
    }
    // en caso de empate en la edad;
    if (edades[j] == edad) {//cuando encuentra dos edades iguales usa la altura para desempatar
